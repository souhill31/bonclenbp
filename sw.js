const CACHE = 'souhill-v102';
const ASSETS = ['/bonclenbp/', '/bonclenbp/index.html', '/bonclenbp/manifest.json', '/bonclenbp/icons/icon-192.png', '/bonclenbp/icons/icon-512.png'];
...
self.addEventListener('notificationclick', e => {
  ...
  e.waitUntil(
    clients.matchAll({type:'window', includeUncontrolled:true}).then(function(cls){
      ...
      else { clients.openWindow('/bonclenbp/'); }
    })
  );
});
  ...
  for(const name of names){
    ...
    if(diffMin >= 0 && diffMin < 2){
      ...
      await self.registration.showNotification('🕌 '+name+' — Heure de la prière', {
        ...
        icon: '/bonclenbp/icons/icon-192.png',
        badge: '/bonclenbp/icons/icon-192.png',
